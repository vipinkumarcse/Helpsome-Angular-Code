...
@Component({
  selector: 'app-add-assignment-customer',
  templateUrl: './add-assignment-customer.component.html',
  styleUrls: ['./add-assignment-customer.component.scss']
})
export class AddAssignmentCustomerComponent implements OnInit {
  ...
  selectedAreaField;
  messageUnseenVal=0;
  shareUrl: any = 'Hey, I have an invite for you to join Assignment with us. 😊 Here is the link! https://dev.hjelpsom.app/#/splashscreen';
  copycontent: string;
  image_url: any=[];
  websiteUrl: string;
  multiple_image:any=[]
  constructor(public socketService: SocketService,private http: HttpClient, private fbb: FacebookService, private fb: FormBuilder, public apiList: ApilistService, private authenticationService: AuthenticationService, private router: Router,private _clipboardService: ClipboardService) {
  ...
  publishData() {
    ...
    this.authenticationService.createAssignment(formData).subscribe((res) => {
      if (!res.isError) {
...
this.copycontent=this.shareUrl+'.Assignment - 1) Headline : '+this.headline+'. 2) Description : '+this.description;
...
this.getCompanyList(this.assignmentArea,this.assignmentService);
        ...
        this.increementStep();
        document.getElementById("step5content").scrollTop = 0;
...
if(arrayCompany){
  if(arrayCompany.length>0 && arrayCompany.length<5){
    const body = document.getElementsByTagName('body')[0];
    body.classList.add('loader_active');
    this.isLoading=true;
    ...
    this.authenticationService.openSnackBar("Please select at least 1 and at the most 4 company(s)", 'Done', '5000','red-snackbar' ,'end','center'
);
  }
}
else{
  ...
  sharefacebookUrl() {
    this.websiteUrl='https://dev.hjelpsom.app'
   for (var i=0;i<this.assignmentCreated.medias.length;i++){
     this.image_url=this.baseImgUrlCust+ 'assignment_images/'+ this.assignmentCreated.medias[i].media
     console.log(this.image_url)
     this.multiple_image.push(this.image_url)
     console.log(this.image_url)
     var new_url =  this.websiteUrl + this.image_url
     console.log(new_url)
     ...
     const params: UIParams = {
      ...
      method: 'share',
      quote:'New Assignment',
    
    };

 
    this.fbb.ui(params)
      .then((res: UIResponse) => console.log(res, "resssss"))
...
//}
  ...
  copied(event) {
    ...
    console.log(event);
    console.log(event.content.model);
    if (event.isSuccess == true) {

      this._clipboardService.copy(event.content.model);
      ...
      selBox.value = event.content.model;
      document.body.appendChild(selBox);
      selBox.focus();
      selBox.select();
      document.execCommand('copy');
      document.body.removeChild(selBox);
      this.authenticationService.openSnackBar('Copied!', 'Done', '5000','blue-snackbar' ,'end','center'
);
    }
  }
  ...
  copyInputMessage(inputElement) {  
        ...
        document.execCommand('copy');  
        this.authenticationService.openSnackBar('Copied!', 'Done', '5000','red-snackbar' ,'end','center'
);
